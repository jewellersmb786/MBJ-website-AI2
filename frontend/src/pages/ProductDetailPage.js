...
const ProductDetailPage = () => {
  ...
  const handleKeyDown = useCallback((e) => {
    if (!lightboxOpen || imgCount === 0) return;
    if (e.key === 'ArrowRight') setLightboxIdx(i => (i + 1) % imgCount);
    if (e.key === 'ArrowLeft') setLightboxIdx(i => (i - 1 + imgCount) % imgCount);
  ...
  const waLink = () => {
    ...
    const msg = `Hi, I'm interested in ${product.name} (Weight: ${product.weight}g, Purity: ${(product.purity || '').toUpperCase()}). Could you share more details?`;
    ...
    <div style={pageStyle} onContextMenu={e => { if (e.target.tagName === 'IMG') e.preventDefault(); }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '100px 32px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '32px', fontSize: '12px', color: 'rgba(255,255,255,0.35)' }}>
          ...
          <span>›</span>
          <span style={{ color: 'rgba(255,255,255,0.5)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '200px' }}>{product.name}</span>
        </div>
        ...
        <div className="pdp-grid" style={{ display: 'grid', gridTemplateColumns: '60% 40%', gap: '48px', alignItems: 'start' }}>
          ...
          <div>
            {/* Main image */}
            <div
              style={{ position: 'relative', background: '#1a0710', borderRadius: '8px', overflow: 'hidden', aspectRatio: '4/5', marginBottom: '12px', cursor: imgCount ? 'zoom-in' : 'default' }}
              onClick={() => imgCount && openLightbox(mainIdx)}
            >
              {imgCount > 0 ? (
                <img src={images[mainIdx]} alt={product.name} draggable={false}
                  style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block', userSelect: 'none' }}
                  onContextMenu={e => e.preventDefault()}
                />
              ) : (
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ fontFamily: 'Georgia, serif', fontSize: '80px', color: 'rgba(212,175,55,0.15)' }}>{product.name.charAt(0)}</span>
                </div>
              )}
              {imgCount > 0 && (
              ...
              <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '4px' }}>
                {images.map((img, i) => (
                  <button key={i} onClick={() => setMainIdx(i)}
                    style={{ flexShrink: 0, width: '72px', height: '72px', padding: 0, border: `2px solid ${mainIdx === i ? '#D4AF37' : 'rgba(255,255,255,0.12)'}`, borderRadius: '6px', overflow: 'hidden', background: '#1a0710', cursor: 'pointer', transition: 'border-color 0.2s' }}
                  >
          ...
          <div style={{ paddingTop: '8px' }}>
            ...
            {/* Name */}
            <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(22px, 2.5vw, 30px)', fontWeight: 400, color: '#D4AF37', margin: '0 0 20px', lineHeight: 1.2 }}>
              {product.name}
            </h1>
            ...
            <a href={waLink()} target="_blank" rel="noopener noreferrer"
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', padding: '16px 24px', background: '#22c55e', color: '#fff', borderRadius: '10px', textDecoration: 'none', fontSize: '15px', fontWeight: 700, transition: 'background 0.2s' }}
                  ...
                  <Link to={`/product/${sim.id}`} style={{ textDecoration: 'none', display: 'block' }}>
                    ...
                    >
                      <div style={{ aspectRatio: '1', overflow: 'hidden', background: '#120808' }}>
                        {sim.images?.[0] ? (
                          <img src={sim.images[0]} alt={sim.name} draggable={false}
                      ...
                      <div style={{ padding: '12px 14px' }}>
                        <h3 style={{ fontFamily: 'Georgia, serif', fontSize: '14px', fontWeight: 400, color: '#D4AF37', margin: '0 0 4px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{sim.name}</h3>
      ...
      <AnimatePresence>
        {lightboxOpen && imgCount > 0 && (
          ...
          >
            ...
            {imgCount > 1 && (
              <button onClick={e => { e.stopPropagation(); setLightboxIdx(i => (i - 1 + imgCount) % imgCount); }}
                style={{ position: 'absolute', left: '20px', top: '50%', transform: 'translateY(-50%)', width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
                <ChevronLeft size={22} />
              </button>
            )}
            <motion.img
              ...
              src={images[lightboxIdx]} alt={product.name} draggable={false}
              style={{ maxWidth: '90vw', maxHeight: '90vh', objectFit: 'contain', borderRadius: '4px', userSelect: 'none' }}
              onClick={e => e.stopPropagation()}
              onContextMenu={e => e.preventDefault()}
            />
            {imgCount > 1 && (
              <button onClick={e => { e.stopPropagation(); setLightboxIdx(i => (i + 1) % imgCount); }}
                style={{ position: 'absolute', right: '20px', top: '50%', transform: 'translateY(-50%)', width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
                <ChevronRight size={22} />
              </button>
            )}
            {imgCount > 1 && (
              <div style={{ position: 'absolute', bottom: '20px', left: '50%', transform: 'translateX(-50%)', fontSize: '12px', color: 'rgba(255,255,255,0.5)', letterSpacing: '0.1em' }}>
